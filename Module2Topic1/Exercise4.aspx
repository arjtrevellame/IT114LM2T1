...
<body>
    ...
    <form runat="server">
        ...
        <asp:RequiredFieldValidator ID="valFullName" ErrorMessage="Full name is required!" ControlToValidate="fullName" runat="server"/><br/>
        ...
        <asp:RangeValidator ErrorMessage="Number must be between 0 and 100." ControlToValidate="age" runat="server" MaximumValue="100" MinimumValue="0" /><br/>
        ...
        <asp:RegularExpressionValidator ErrorMessage="Invalid email format" ControlToValidate="email" ValidationExpression="^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$" runat="server" /><br/>
        ...
        <asp:CompareValidator ID="compareValidator" ErrorMessage="Emails do not match" ControlToValidate="confirmEmail" ControlToCompare="email" runat="server" /><br />

        <%-- Todo 4.5 When the user submits the form and all validations pass, display all the inputted texts in the "result" label --%>
        <asp:Button Text="Submit" runat="server" OnClick="Submit_Click" UseSubmitBehavior="true" /><br /><br />
        Result: <br /><asp:Label ID="result" runat="server" Text=""/><br />
